...
const ChromaGrid = forwardRef<ChromaGridHandle, ChromaGridProps>(({
  items,
  className = "",
  radius = 300,
  damping = 0.45,
  ...
  // internal animated move (same as before)
  ...
  // pointer handlers still exist if the user interacts directly with ChromaGrid
  const handleMove = (e: React.PointerEvent) => {
    const r = rootRef.current!.getBoundingClientRect();
    moveTo(e.clientX - r.left, e.clientY - r.top);
    gsap.to(fadeRef.current, { opacity: 0, duration: 0.22, overwrite: true });
  };
  ...
  // Expose imperative handle: setPointer(x,y) and resetPointer()
  useImperativeHandle(ref, () => ({
    setPointer: (x: number, y: number) => {
      // set directly without starting a tween (for snappy card hover)
      const el = rootRef.current;
      if (!el) return;
      el.style.setProperty("--x", `${x}px`);
      el.style.setProperty("--y", `${y}px`);
      // also hide the fade layer quickly
      if (fadeRef.current) gsap.to(fadeRef.current, { opacity: 0, duration: 0.12, overwrite: true });
    },
    resetPointer: () => {
      const el = rootRef.current;
      if (!el) return;
      ...
      const cx = width / 2;
      const cy = height / 2;
      // tween back to center slowly
      gsap.to(pos.current, {
        x: cx, y: cy, duration: fadeOut, ease,
        onUpdate: () => { setX.current?.(pos.current.x); setY.current?.(pos.current.y); },
        overwrite: true
      });
      if (fadeRef.current) gsap.to(fadeRef.current, { opacity: 1, duration: fadeOut, overwrite: true });
    }
  }), [fadeOut, ease, damping]);

  // keep the old showCards rendering if requested (you likely won't use it)
  return (
    <div
      ref={rootRef}
      ...
      style={{ /* initial CSS vars; coordinate values will be px strings */ } as React.CSSProperties}
      data-chroma-root
    >
      {showCards && data.map((c, i) => (
        <article key={i} className="group relative flex flex-col w-[300px] rounded-[20px] overflow-hidden border-2 border-transparent transition-colors duration-300 cursor-pointer"
          style={{ background: c.gradient, ["--spotlight-color" as any]: "rgba(255,255,255,0.3)" } as React.CSSProperties }
        >
          <div className="absolute inset-0 pointer-events-none transition-opacity duration-500 z-20 opacity-0 group-hover:opacity-100"
            style={{ background: 'radial-gradient(circle at var(--mouse-x) var(--mouse-y), var(--spotlight-color), transparent 70%)' }} />
          <div className="relative z-10 flex-1 p-[10px] box-border">
            <img src={c.image} alt={c.title} loading="lazy" decoding="async" className="w-full h-full object-cover rounded-[10px]" />
          </div>
      ...
      <div ref={fadeRef} className="absolute inset-0 pointer-events-none transition-opacity duration-[250ms] z-40" style={{
        backdropFilter: 'grayscale(1) brightness(0.78)',
        WebkitBackdropFilter: 'grayscale(1) brightness(0.78)',
        background: 'rgba(0,0,0,0.001)',
