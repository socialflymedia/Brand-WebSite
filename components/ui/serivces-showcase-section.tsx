...
const services: Service[] = [
  {
    id: 1,
    title: "AI & Automation",
    description: "Agentic AI agents & enterprise automation",
    icon: Brain,
...
const ServiceCard: React.FC<{ service: Service; index: number }> = ({ service, index }) => {
  ...
  const motionProps = {
    ...
    whileHover: isLink ? { y: -4, transition: { duration: 0.2 } } : undefined,
  ...
  const inner = (
    <div className="relative h-full p-4 rounded-xl bg-white/5 backdrop-blur-sm border border-gray-800/50 group-hover:border-orange-500/30 transition-all duration-300">
      {/* Hover glow */}
      <div
      ...
      {/* Content */}
      <h3 className={`text-white font-semibold text-sm mb-1 transition-colors ${isLink ? "group-hover:text-orange-400" : ""}`}>
        {service.title}
      </h3>
  ...
  if (isLink) {
    return (
      <motion.a href={service.href} {...motionProps}>
        {inner}
      </motion.a>
    );
  }
    ...
    <section className="relative py-16 lg:py-20 bg-black overflow-hidden">
      ...
      <div className="relative z-10 container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <a
            href="/services"
            className="inline-flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-orange-500 to-orange-600 text-white text-sm font-medium rounded-lg hover:shadow-lg hover:shadow-orange-500/20 transition-all"
          >
