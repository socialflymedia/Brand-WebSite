...
/* =========================
   ServiceCard - ONLY SIZE CHANGED, STYLING KEPT EXACT
   ========================= */
const ServiceCard: React.FC<{ service: Service; onExplore?: (s: Service) => void }> = React.memo(
  ({ service, onExplore }) => {
    ...
    const getHeight = () => {
      ...
      if (window.innerWidth < 1024) return 560;
      return 580; // Desktop - smaller than 720
    };
    ...
    return (
      ...
      >
        {/* subtle single-color glow - EXACT SAME */}
        <div
          aria-hidden
        ...
        {/* image top (approx 60%) - EXACT SAME */}
        ...
        {/* content below image - EXACT SAME STYLING */}
...
/* =========================
   Main ServicesSection - EXACT SAME STYLING & FUNCTIONALITY
   ========================= */
export default function ServicesSection(): JSX.Element {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const isAutoRef = useRef<boolean>(true);
  const speedRef = useRef<number>(90); // SAME SPEED
  const prefersReduced = useReducedMotion();

  // doubled for seamless loop - EXACT SAME
  const doubled = useMemo(() => [...services, ...services], []);

  // RAF auto-scroll - EXACT SAME
  useEffect(() => {
  ...
  // pause/resume helpers - EXACT SAME
  ...
  // manual scroll by card - ADJUSTED FOR NEW SIZES
  const scrollByCard = useCallback((direction: "left" | "right") => {
    const el = containerRef.current;
    if (!el) return;
    const cardWidth = window.innerWidth < 640 ? 300 : window.innerWidth < 768 ? 340 : window.innerWidth < 1024 ? 380 : 420;
    const gap = 64; // SAME GAP
    const delta = direction === "left" ? -(cardWidth + gap) : cardWidth + gap;
    isAutoRef.current = false;
    el.scrollBy({ left: delta, behavior: "smooth" });
  }, []);

  // go to index in primary set - ADJUSTED FOR NEW SIZES
  ...
  // pause on focusin/out for keyboard users - EXACT SAME
  useEffect(() => {
    const el = containerRef.current;
    if (!el) return;
